...
#   GITHUB_REPOSITORY  - e.g., hydrolix/hydrolix-ai-toolkit
...
if [ -f "$SITE_DIR/skills.json" ]; then
    TIMESTAMP=$(sed -n 's/^  "lastUpdated": "\(.*\)",$/\1/p' "$SITE_DIR/skills.json" | head -1)
fi
TIMESTAMP=${TIMESTAMP:-$(date -u +"%Y-%m-%d %H:%M:%S UTC")}
REPO_URL="https://github.com/${GITHUB_REPOSITORY:-hydrolix/hydrolix-ai-toolkit}"
...
<head>
    ...
    <title>Hydrolix AI Toolkit</title>
    ...
    <div class="container">
        <div class="install-section">
            ...
            <div id="tab-claude" class="install-content active">
                <pre><code>claude plugin install hydrolix/hydrolix-ai-toolkit</code></pre>
            </div>
