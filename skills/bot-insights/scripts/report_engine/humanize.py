...
# Reader-facing plurals for entity-type nouns. Naive "+ s" produces
# "AI categorys" / "countrys" / "user agents" — the last is fine but the
# first two read wrong. Unknown identifiers fall back to "{label}s".
...
def humanize_entity_type_plural(s: str) -> str:
    """Reader-facing plural noun for an entity_type identifier.

    Falls back to ``humanize_entity_type(s) + "s"`` for unknown identifiers.
    Used wherever the singular label would naively be ``label + "s"`` —
    triage strip headings, action-summary parentheticals, etc.
    """
    if s in ENTITY_TYPE_PLURAL_LABELS:
        return ENTITY_TYPE_PLURAL_LABELS[s]
    return humanize_entity_type(s) + "s"


def title_case_label(value: Any) -> str:
    """Title-case a label while preserving acronyms (ASN, IP, AI, SIEM).

    Use as a Jinja2 filter where the legacy ``capitalize`` would lowercase
    embedded acronyms — e.g. ``"ASN" | capitalize`` becomes ``"Asn"``,
    ``"AI category" | capitalize`` becomes ``"Ai category"``. This filter
    title-cases every word, preserves acronyms, and keeps already-uppercase
    tokens as-is (so producer-supplied identifiers like ``SIEM`` survive).
    """
    ...
    for token in text.split(" "):
        if not token:
            ...
            continue
        lower = token.lower()
        if lower in _DISPLAY_LABEL_ACRONYMS:
...
def humanize_entity_value(value: Any, entity_type: str | None = None) -> str:
    """Reader-facing rendering of an entity *value* (not the entity_type).

    Entity values are producer-controlled identifiers — ASN numbers, host
    names, AI-category slugs, country codes. snake_case slugs (e.g.
    ``ai_training``) should display with proper Title Case + acronym
    preservation. ASN values get the ``"ASN "`` prefix here so callers don't
    have to special-case it.
    """
    text = stringify(value)
    if not text:
        return text
    if entity_type == "client_asn":
        return f"ASN {text}"
